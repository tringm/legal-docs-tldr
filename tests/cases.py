...
class CompareTestCase(BaseTestCase):
    ...
    def compare_file(self, out_file_path: Path, exp_file_path: Path, msg=None):
        ...
        if not msg:
            self.assertTrue(
                filecmp.cmp(str(out_file_path), str(exp_file_path), shallow=False),
                f"out file {out_file_path!s} does not match exp file {exp_file_path!s}",
            )
        else:
    ...
    def compare_json_files(
        self, out_file_path: Path, exp_file_path: Path, compare_order: bool = False, is_ndjson_file: bool = False
    ):
        self.logger.debug(f"Comparing {out_file_path} with {exp_file_path}")
