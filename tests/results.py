...
class TestResultLogMetrics(unittest.TextTestResult):
    ...
    def addFailure(self, test, err):
        ...
        if self.metrics_log_path:
            with self.metrics_log_path.open(mode="a+") as f:
                f.write(
                    f"{getattr(test, '_started_at', '')}, {test.id()}, fail, {getattr(test, '_elapsed', '')}, "
                    f"{err}\n"
                )
    ...
    def addError(self, test, err):
        ...
        if self.metrics_log_path:
            with self.metrics_log_path.open(mode="a+") as f:
                f.write(
                    f"{getattr(test, '_started_at', '')}, {test.id()}, error, {getattr(test, '_elapsed', '')}, "
                    f"{err}\n"
                )
...
class TestResultCompareFileMeld(TestResultLogMetrics):
    def addFailure(self, test, err):
        super().addFailure(test, err)
        out_file_path = test.out_file_path
        exp_file_path = test.exp_file_path
