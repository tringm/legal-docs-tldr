...
class TosdrDataPreprocessor:
    ...
    @log_exec_time(LOG)
    def load_service_data_by_file(self, service_file_path: Path, reload=False, service_name: str = None):
        ...
        if service_name not in self._loaded_services_data or reload:
            try:
                ...
                LOG.error(f'failed to load `{service_name}` data file {service_file_path}: {e}')
                raise e
            self._loaded_services_data[service_name] = data
        else:
            LOG.info(f'`{service_name}` data existed')
    ...
    def load_all_services_data_in_folder(self, folder_path: Path):
        ...
        for file_path in folder_path.glob("*.json"):
            try:
                self.load_service_data_by_file(file_path)
    ...
    def service_data(self, service_name: str):
        data = self._loaded_services_data.get(service_name)
        if not data:
            raise ValueError(f'`{service_name}` data not loaded')
        return data
    ...
    def service_quote_text_and_summary(self, service_name) -> List[Tuple[Optional[str], Optional[str]]]:
        ...
        """
        service_data = self.service_data(service_name)
        service_point_data = service_data.get('pointsData')
        if not service_point_data:
            raise RuntimeError(f'`{service_name}` pointsData not found')
